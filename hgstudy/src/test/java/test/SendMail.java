...
public class SendMail {
    ...
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail) throws Exception {
        ...
        // 4. Subject: 邮件主题（避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
        message.setSubject("请验证", "UTF-8");

        // 5. Content: 邮件正文（可以使用html标签）（避免被邮件服务器误认为是滥发广告以至返回失败，请修改发送内容）
        message.setContent("你本次的验证码为123456", "text/html;charset=UTF-8");
