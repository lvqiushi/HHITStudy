...
<div class="panel panel-default">
    <div class="panel-heading">课件列表</div>
		<div class="panel-body text-center">
			<p class="text-left">课件总数 : ${number}</p>
			<table class="table kjTable">
				...
				<c:forEach var="c" items="${pagebean.contents}">
					<tr>
						...
						<td>${c.time }</td>
						<td>${c.uploader }</td>
						<td><a href="kejian!delete?couresewareid=${c.fileId}&couid=${couid}">删除</a> </td>
					</tr>
				</c:forEach>
			</table>
		</div>
		...
		<ul class="pagination">
			<li><a href="kejian?couid=${couid}&curpage=1">首页</a></li>

			<c:if test="${curpage>1}">
				<li><a href="kejian?couid=${couid}&curpage=${curpage-1}"
			...
			<c:forEach begin="${pagebean.star}" end="${pagebean.end}" var="i">
				...
				<c:if test="${curpage!=i}">
					<li><a href="kejian?couid=${couid}&curpage=${i}"
			...
			<li><a
				href="kejian?couid=${couid}&curpage=${pagebean.pageCount}">末页</a></li>
		</ul>
		</nav>
		</c:if>
		<form class="form-horizontal" action="KejianUpload" role="form" method="post" enctype="multipart/form-data" onsubmit="return checkcourse();">
    	<input name="couid" type="hidden" value="${couid }" />
        <div class="form-group">
            <label for="add-kj" class="col-sm-2 control-label">添加课件</label>
            <div class="col-sm-10">
                <input type="file" name="uploadFile" class="form-control" id="add-kj" >
            </div>
        </div>
        <div class="form-group">
