...
<%@ page language="java" import="hiber.pojo.*"
		...
		<div class="panel panel-default">
			<div class="panel-heading">
				课件列表 <span class="pull-right"><a
					href="CourseAction?couID=${couid}">返回</a></span>
			</div>
			<div class="panel-body">
				<table class="table">
					...
					<c:forEach var="c" items="${pagebean.contents}">
						<tr>
							<td><a href="download!download.action?id=${c.fileId}">${c.fileName}</a></td>
			...
			<c:if test="${not empty pagebean}">
				...
				<ul class="pagination">
					<li><a href="showkejian?couid=${couid}">首页</a></li>

					<c:if test="${curpage>1}">
						<li><a href="showkejian?couid=${couid}&curpage=${curpage-1}"
					...
					<c:forEach begin="${pagebean.star}" end="${pagebean.end}" var="i">
						...
						<c:if test="${curpage!=i}">
							<li><a href="showkejian?couid=${couid}&curpage=${i}"
					...
					<li><a
						href="showkejian?couid=${couid}&curpage=${pagebean.pageCount}">末页</a>
					</li>
				</ul>
				</nav>
			</c:if>
